...
By default, the BOM(s) will be attached as an additional artifacts with `cdx.xml` or `cdx.json` extension during a Maven `install` or `deploy`:

* `${project.artifactId}-${project.version}.cdx.xml`
* `${project.artifactId}-${project.version}.cdx.json`

Notice: until version 2.9 of this plugin, attachment was done as `-cyclonedx.xml` or  `-cyclonedx.json`
